...
class DPODataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            inputs_pair.append([prompt, chosen])
            rejected = {}
            for key in feature:
        ...
        # Add batch_tokens to the collated results
        collated_batch["tokens"] = {
            "input_ids": batch_tokens["input_ids"],
            "attention_mask": batch_tokens["attention_mask"],
            "token_type_ids": batch_tokens["token_type_ids"]  # Some tokenizers might not return this
        }

        collated_batch["seq"] = torch.stack([torch.tensor(sample['seq']) for sample in features], dim=0)
        collated_batch["cans"] = torch.stack([torch.tensor(sample['cans']) for sample in features], dim=0)
        collated_batch["len_seq"] = torch.stack([torch.tensor(sample['len_seq']) for sample in features], dim=0)
        collated_batch["item_id"] =  torch.stack([torch.tensor(sample['item_id']) for sample in features], dim=0)
        collated_batch["len_cans"] =  torch.stack([torch.tensor(sample['len_cans']) for sample in features], dim=0)
