...
def main(args):
    ...
    if args.stage == 'teacher_sft':
        ...
        if args.ckpt_path:
            ...
            # new_state_dict = {k.replace('student_llama_model.', 'llama_model.'): v for k, v in ckpt['state_dict'].items()}
            # # new_state_dict = {k.replace('student_projector.', 'projector.'): v for k, v in new_state_dict.items()}
            # model.load_state_dict(new_state_dict, strict=False)

        data_module = DInterface(llm_tokenizer=model.llama_tokenizer,**vars(args))

    elif args.stage == 'student_sft':
        model = MInterface(**vars(args))
        if args.ckpt_path:
            ckpt = torch.load(args.ckpt_path, map_location='cpu')
        ...
        if args.ckpt_head_path:
            ...
            # print('step', model.teacher_llama_model.state_dict().keys())
        ...
        if args.student_ckpt_path:
            ...
            model.student_projector.load_state_dict(new_state_dict1, strict=True)
            # model.load_state_dict(ckpt['state_dict'], strict=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage', default='stage1', choices=['teacher_sft', 'student_sft', 'distillation_stage_prelim', 'distillation_stage1', 'distillation_stage2'], type=str)
