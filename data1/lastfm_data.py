...
if __name__ == "__main__":
    ...
    train_data = LastfmData('/data/mcy/LLaRA/hf_data/lastfm', 'train')
    
    movie_id2name = train_data.get_music_id2name()
    train_data = train_data.session_data4frame('/data/mcy/LLaRA/hf_data/lastfm/train_data.df', movie_id2name)
    
    item_play_count = train_data['next_item_name'].value_counts().reset_index()
    item_play_count.columns = ['next_item_name', 'play_count']

    median_play_count = item_play_count['play_count'].median()
    popular_items = item_play_count[item_play_count['play_count'] >= median_play_count]
    lesspopular_items = item_play_count[item_play_count['play_count'] < median_play_count]

    pop_item_dict = {item: count for item, count in zip(popular_items['next_item_name'], popular_items['play_count'])}
    lesspop_item_dict = {item: count for item, count in zip(lesspopular_items['next_item_name'], lesspopular_items['play_count'])}

    with open('popular_group.pkl', 'wb') as file:
        pickle.dump(pop_item_dict, file)

    with open('less_popular_group.pkl', 'wb') as file:
        pickle.dump(lesspop_item_dict, file)